...
public class ModularArithmetics {
    ...
    private static void powerFun(int a, int n, int p){
        // (2 ^ 5 ) % 7
    ...
    private static void calculateNModP(){
        String N = "43535321";
        int p = 47;
        int k = N.length();
        long ans = 0;
    ...
    private static void excelColumnNumber(){
        String A = "ABCD";
        System.out.println('Z' - 65 + 1);
        int sum = 0;
        for(int i=A.length()-1, k=0; i>=0; i--, k++){
            sum = (int) (sum + (A.charAt(k) - 65 + 1) * Math.pow(26, i));
        }
        System.out.println(sum);
    }

    private static void gretestPossiblePositiveM(){
        int A = 106978;
        int B = 4487506;
        int limit = A > B ? A - B : B - A;
        int ans = 1;
    ...
    private static void divisibilityOf8(){
        String A = "344";
        int last3Digits = Integer.parseInt(A);
        if(A.length() > 3){
            last3Digits = Integer.parseInt(A.substring(A.length()-3));
        }
        System.out.println(last3Digits % 8 == 0);

    }

    private static void repeatedAndMissingNumbers(){
    ...
    private static void concatenateThreeNumbers(){
        int A = 55;
        int B = 43;
        int C = 47;
        ...
        if( A == smallest && B <= C){
            ans = ans + B + C;
        }else if(A == smallest && B > C){

        }
    }

    private static void repearMissing(){
    ...
    private static void lcmOfTwoNumbers(){
        int A = 9;
        int B = 6;
        int ans = 1;
        for(int i=1; i<A*B; i++){
